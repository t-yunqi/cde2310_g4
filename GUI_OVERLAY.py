...
FONT_SCALE     = 0.65
FONT_THICKNESS = 2
CORNER_LEN     = 18              # length of corner tick marks
WINDOW_TITLE   = "ArUco Overlay  |  q = quit"
# ─────────────────────────────────────────────────────────────────────────────
...
class ArucoOverlayNode(Node):
    def __init__(self):
        ...
        self.markers     = []          # list of (marker_id, pose) from /aruco_markers
        self.marker_size = 0.0625      # metres — match your aruco_node marker_size param

        # Camera intrinsics (filled once camera_info arrives)
        ...
        self.get_logger().info("aruco_overlay started — waiting for camera…")
    ...
    def _image_cb(self, msg: CompressedImage):
        try:
            ...
            frame = cv2.imdecode(buf, cv2.IMREAD_COLOR)  # always gives bgr8
            if frame is None:
    ...
    def get_annotated_frame(self):
        """Return a copy of the latest frame with bounding boxes drawn."""
        with self.lock:
        ...
        # Fallback intrinsics if camera_info hasn't arrived yet
        if fx is None:
            fx = fy = w * 1.2
            cx, cy = w / 2.0, h / 2.0
            dist = np.zeros((4, 1))
        ...
        half = msize / 2.0
        # 3-D marker corners in marker frame (z=0 plane)
        ...
        for marker_id, pose in markers:
            ...
            # Draw the actual projected corner dots
            ...
            cv2.arrowedLine(frame, origin_2d, z_pt,
                            color=(255, 80, 0),    # blue-ish arrow
                            thickness=2,
                            tipLength=0.3)
            # Z value label sitting ON TOP of the arrow tip
            depth = float(pose.position.z)
            z_label = f"Z: {depth:.3f}m"
            (tw, th), _ = cv2.getTextSize(z_label, FONT, 0.55, 1)
            lx = z_pt[0] - tw // 2
            ly = z_pt[1] - 8
            cv2.rectangle(frame, (lx - 2, ly - th - 2), (lx + tw + 2, ly + 2),
                          (0, 0, 0), -1)
            cv2.putText(frame, z_label, (lx, ly),
...
def _pose_to_rvec_tvec(pose):
    """Convert a geometry_msgs/Pose to OpenCV rvec + tvec."""
    # Translation
    tvec = np.array([[pose.position.x],
                     [pose.position.y],
                     [pose.position.z]], dtype=np.float64)

    # Quaternion → rotation matrix → rvec
    q = pose.orientation
...
def _draw_corner_ticks(img, x1, y1, x2, y2, color, length):
    """Draw short tick marks at each corner of the bounding box."""
    t = BOX_THICKNESS
